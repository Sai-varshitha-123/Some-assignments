...
<script>
	...
	function addCart(name,price)
	{
		...
		if(!loggedInUser){
			...
			//let storedData = localStorage.getItem("id_" + loggedInUser);
		    //let existingProducts = storedData ? JSON.parse(storedData) : {}; 
		    var count=1;
		     var existingProducts = { productName: name, productPrice: price };
		    localStorage.setItem("id_" + count, JSON.stringify(existingProducts));
			count=count+1;
		}
	}
	function gotocart()
		...
		<section>
			<% for(Product product : products) { %>
				<div class="card">
					...
					<h3 id="price">Product Price: $<%= product.getPrice() %></h3>
					<button onclick="addCart('<%=product.getName() %>','<%=product.getPrice() %>')">Add to Cart</button>
				</div>
			<% } %>
		</section>
	</div>
